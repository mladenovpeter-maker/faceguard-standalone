 ...
 *   Worker → Main:  { id: number; descriptors: number[][] }
 *                 | { id: number; error: string }
 */
import { parentPort } from "worker_threads";
import { computeAllFaceDescriptors } from "./face-recognition.js";
...
parentPort.on("message", async (msg: { id: number; imageBuffer: ArrayBuffer }) => {
  ...
  try {
    ...
    const descriptors = await computeAllFaceDescriptors(buffer);
    parentPort!.postMessage({ id, descriptors });
  } catch (err) {
